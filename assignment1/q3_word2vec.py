...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    cost = -np.log(y[target]) #target is one-hot
    delta=y
    delta[target]-=1
    N=delta.shape[0]
    D=predicted.shape[0]
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors, 
    ...
    ### YOUR CODE HERE
    '''for i in xrange(C):
        cost,gradPred,grad = word2vecCostAndGradient(inputVectors,tokens[currentWord],outputVectors)
        currentWord = contextWords[i+1]
    gradIn=gradPred
    gradOut=grad'''
    ### END YOUR CODE
